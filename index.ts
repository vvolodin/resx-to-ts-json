...
export function executeResxToJson(virtualResxFolder: string, virtualJsonFolder: string): void {
    let files = getFilesFromFolder(virtualResxFolder);

    if (files !== undefined && files !== null) {
        for (let i = 0, length = files.length; i < length; i++) {   
            const resxFilename = files[i];
            convertResxToJson(resxFilename, virtualJsonFolder);
        }
    }
}
...
function convertResxToJson(resxFilename: string, virtualJsonFolder: string): void {
    fs.readFile(resxFilename, function(err: any, data: any) {
        const parser = new xml2js.Parser();

        parser.parseString(data, function (err: any, result: any) {
            if (result !== undefined) {
                convertXmlToJsonFile(result, resxFilename, virtualJsonFolder);                       
            }
        });  
    });    
}
...
function convertXmlToJsonFile(xmlObject: any, resxFilename: string, virtualJsonFolder: string): void {
    const projectRoot = getProjectRoot();
    const relativeResxFilename = resxFilename.replace(projectRoot, "").replace(/\\/g, "/");
    ...
    if (Object.keys(dictionary).length > 0) {
        ...
        if (virtualJsonFolder === undefined || virtualJsonFolder === '') {
            ...
            // Write the file to the given output folder.
            const jsonFileNameWithoutPath = jsonFileName.substr(jsonFileName.lastIndexOf('\\') + 1);
